...
export default function reducer(state = inicialState, action) {
  switch (action.type) {
    ...
    case FILTER_BY_NAME:
      return {
        ...state,
        actionsState: state.allGames.filter((ele) =>
    ...
    case FILTER_BY_GENRES:
      return {
        ...
        actionsState:
          action.payload === "ALL"
            ? state.allGames
            : state.actionsState.filter(
                (e) => e.Genres.filter((e) => e.name == action.payload).length
              ),
      };
...
low */
      ...
      switch (action.payload) {
        case "UP":
          order = state.actionsState.sort(function (a, b) {
            if (a.name > b.name) {
              return 1;
            }
            if (a.name < b.name) {
